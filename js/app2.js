...
Key.prototype.update = function() {
    if (allKeys.length > 1) { // only check if there is more than one key on canvas
        if (allKeys[0].x === allKeys[1].x && allKeys[0].y === allKeys[1].y) {
            if(allKeys[0].y <300) {
                allKeys[1].y += 83; // change the y coordinate of the second key if they spawn in same location 
            } 
            else {
                allKeys[1].y -=83; 
            }
...
var Star = function() {
    this.x = this.x_position();
    this.y = 40;
    this.sprite = 'images/shenron.png';
}
...
var selectChar = function() {
    ...
    this.char_arr = ['images/goku.png', 'images/vegeta.png', 'images/gohan.png', 'images/trunks.png', 'images/piccolo.png'];
...
GameOver.prototype.render = function() {
    if (game_state == 'end') {
        ...
        ctx.drawImage(Resources.get(player.sprite), 205, 455)
    }
}
